...
class General_model extends CI_Model
{
    ...
    public function deleteByStatus($table, $where, $slave = true)
    {
        $this->db->where($where);
        $updated_data['astatus'] = 0;
        if ($slave) {
            $updated_data['deleted_at'] = date("Y-m-d H:i:s");
        }
        $result = $this->db->update($table, $updated_data);
        return $result;
    }

    public function restoreByStatus($table, $where, $slave = true)
    {
        $this->db->where($where);
        $updated_data['astatus'] = 1;
        if ($slave) {
    ...
    public function deActivateData($table, $where, $slave = true)
    {
        $this->db->where($where);
    ...
    public function activateData($table, $where, $slave = true)
    {
        $this->db->where($where);
